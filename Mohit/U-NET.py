...
train_test=tf.concat([train1,train2,train3,train4,train5,train6,train7],0)
#Stack Segmented Inputs
train_label=tf.concat([trainlab1,trainlab2,trainlab3,trainlab4,trainlab5,trainlab6,trainlab7],0)
...
def FCN_UNet(x):
    # Hyperparameters
    # for choosing weights from a normal distribution
    mu = 0
    sigma = 0.1
    ...
    W1_1 = tf.Variable(tf.truncated_normal(shape=(3, 3, 3, 1, 8), mean=mu, stddev=sigma))
    x = tf.nn.conv3d(x, W1_1, strides=[1, 1, 1, 1, 1], padding='SAME')
    b1_1 = tf.Variable(tf.zeros(8))
    ...
    W1_2 = tf.Variable(tf.truncated_normal(shape=(3, 3, 3, 8, 8), mean=mu, stddev=sigma))
    x = tf.nn.conv3d(x, W1_2, strides=[1, 1, 1, 1, 1], padding='SAME')
    b1_2 = tf.Variable(tf.zeros(8))
    ...
    W2_1 = tf.Variable(tf.truncated_normal(shape=(3, 3, 3, 8, 16), mean=mu, stddev=sigma))
    x = tf.nn.conv3d(x, W2_1, strides=[1, 1, 1, 1, 1], padding='SAME')
    b2_1 = tf.Variable(tf.zeros(16))
    ...
    # TODO: Layer 2 sublayer 2
    W2_2 = tf.Variable(tf.truncated_normal(shape=(3, 3, 3, 16, 16), mean=mu, stddev=sigma))
    x = tf.nn.conv3d(x, W2_2, strides=[1, 1, 1, 1, 1], padding='SAME')
    b2_2 = tf.Variable(tf.zeros(16))
    ...
    W3_1 = tf.Variable(tf.truncated_normal(shape=(3, 3, 3, 16, 32), mean=mu, stddev=sigma))
    x = tf.nn.conv3d(x, W3_1, strides=[1, 1, 1, 1, 1], padding='SAME')
    b3_1 = tf.Variable(tf.zeros(32))
    ...
    W3_2 = tf.Variable(tf.truncated_normal(shape=(3, 3, 3, 32, 32), mean=mu, stddev=sigma))
    x = tf.nn.conv3d(x, W3_2, strides=[1, 1, 1, 1, 1], padding='SAME')
    b3_2 = tf.Variable(tf.zeros(32))
    ...
    W3_3 = tf.Variable(tf.truncated_normal(shape=(3, 3, 3, 32, 64), mean=mu, stddev=sigma))
    x = tf.nn.conv3d(x, W3_3, strides=[1, 1, 1, 1, 1], padding='SAME')
    b3_3 = tf.Variable(tf.zeros(64))
    ...
    # TODO: Layer 3 SubLayer 4
    W3_4 = tf.Variable(tf.truncated_normal(shape=(3, 3, 3, 64, 64), mean=mu, stddev=sigma))
    x = tf.nn.conv3d(x, W3_4, strides=[1, 1, 1, 1, 1], padding='SAME')
    b3_4 = tf.Variable(tf.zeros(64))
    x = tf.nn.bias_add(x, b3_4)
    print("layer 3_2 shape:", x.get_shape())
    ...
    # TODO: Layer 4 Sublayer1
    W4_1 = tf.Variable(tf.truncated_normal(shape=(3, 3, 3, 64, 128), mean=mu, stddev=sigma))
    x = tf.nn.conv3d(x, W4_1, strides=[1, 1, 1, 1, 1], padding='SAME')
    b4_1 = tf.Variable(tf.zeros(128))
    x = tf.nn.bias_add(x, b4_1)
    print("layer 4_1 shape:", x.get_shape())
    ...
    # TODO: Layer 4 Sublayer2
    W4_2 = tf.Variable(tf.truncated_normal(shape=(3, 3, 3, 128, 256), mean=mu, stddev=sigma))
    x = tf.nn.conv3d(x, W4_2, strides=[1, 1, 1, 1, 1], padding='SAME')
    b4_2 = tf.Variable(tf.zeros(256))
    x = tf.nn.bias_add(x, b4_2)
    print("layer 4_2 shape:", x.get_shape())
    ...
    # TODO: layer 5 (final downsample layer)
    W5_1 = tf.Variable(tf.truncated_normal(shape=(1, 1, 1, 256, 512), mean=mu, stddev=sigma))
    x = tf.nn.conv3d(x, W5_1, strides=[1, 1, 1, 1, 1], padding='SAME')
    b5_1 = tf.Variable(tf.zeros(512))
    x = tf.nn.bias_add(x, b5_1)
    print("layer 5_1 shape:", x.get_shape())
    ...
    # TODO: FINAL DOWNSAMPLING LAYER (2 classes) as we want segmented image
    final_down_W = tf.Variable(tf.truncated_normal(shape=(1, 1, 1, 512, 2), mean=mu, stddev=sigma))
    ...
    print("1st UpSample layer:", x.get_shape())
    ...
    # now to upscale to actual image size
    # TODO:3rd Upsample Layer output size of DownSampling layer1
    deconv_shape3 = layer1.get_shape()
    x = tf.contrib.layers.conv3d_transpose(x, deconv_shape3[4], kernel_size=[4, 4, 4], stride=[2, 2, 2])
    print("3rd down sample layer:", x.get_shape())

    # TODO:4TH Upsample Layer output size actual image size
    x = tf.contrib.layers.conv3d_transpose(x, 2, kernel_size=[4, 4, 4], stride=[2, 2, 2])
    logits = x
    print("4th down sample layer:", x.get_shape())
